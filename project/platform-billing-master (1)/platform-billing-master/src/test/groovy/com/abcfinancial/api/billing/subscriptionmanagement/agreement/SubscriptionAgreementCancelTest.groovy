...
class SubscriptionAgreementCancelTest extends BaseTest
{
    ...
    static def agreementNumber = "CBDEFG123456787"
    ...
    void cancelAgreement()
    {
        mvc().perform(put('/cancel-agreement/{agreementNumber}', agreement.agreementNumber).header("Authorization", "Bearer ${bearerToken}")
                .content(toJson(cancelAgreementRequest())))
                .andDo(print()).andDo(document("cancelAgreement"))
    ...
    void cancelAgreements()
    {
        mvc().perform(put('/cancel-agreement/{agreementNumber}', agreement.agreementNumber).header("Authorization", "Bearer ${bearerToken}")
                .content(toJson(cancelAgreementRequests())))
                .andDo(print())
                .andExpect(status().isOk())
                .andReturn()
    }
    ...
    @Test(dependsOnMethods = 'cancelAgreements')
    void getAgreementByNumbers()
    {
        mvc().perform(get('/agreement/{agreementNumber}', agreement.agreementNumber).header("Authorization", "Bearer ${bearerToken}"))
                .andDo(print())
                .andDo(document("getAgreementByNumber"))
